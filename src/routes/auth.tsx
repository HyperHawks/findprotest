...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [{ title: "Sign in — FINDPROTEST" }, { name: "description", content: "Join FINDPROTEST as a follower or leader." }],
  }),
  component: AuthPage,
});
      ...
      <main className="flex-1 grid place-items-center p-6">
        <div className="w-full max-w-md border-2 border-border bg-card p-6 md:p-10 brutal-shadow">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-black tracking-tighter uppercase mb-2">FINDPROTEST</h1>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            >
              {busy ? "Processing..." : mode === "signin" ? "Sign in" : "Join FINDPROTEST"}
            </button>
            <button
              type="button"
              onClick={signinGoogle}
              disabled={busy}
              className="w-full py-4 border-2 border-border bg-background text-foreground font-mono text-[11px] font-extrabold uppercase hover:bg-muted transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
